from config import NUM_WORKERS, PACKAGE_NAME, WARMUP_ITERATIONS, BENCHMARK_ITERATIONS, ITERATION_TIME
...
@Fork(0)
@Warmup(iterations = {WARMUP_ITERATIONS}, time = {ITERATION_TIME}, timeUnit = TimeUnit.SECONDS)
@Measurement(iterations = {BENCHMARK_ITERATIONS}, time = {ITERATION_TIME}, timeUnit = TimeUnit.SECONDS)
...
public class ComplexPolymorphicCallsBenchmark {{
    ...
    public void setup() {{
        ...
        for (long i = num0; i < count; i++) {{
            ...
            int workerId = 1 + random.nextInt({NUM_WORKERS - 1});
